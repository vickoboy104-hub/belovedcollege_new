...
<div {{ $attributes->merge(['class' => 'metric-card stat-gradient-card']) }}
     style="background: {{ $gradient }} !important; border: 1px solid var(--theme-border-soft, rgba(255,255,255,0.14)) !important; border-radius: var(--radius-xl,1rem); padding: 1.25rem 1.4rem; color: var(--dashboard-card-text, #ffffff); position: relative; overflow: hidden; box-shadow: 0 10px 28px rgba({{ $glowRgb }},0.25), 0 2px 8px rgba(0,0,0,0.12); transition: transform 0.2s ease, box-shadow 0.2s ease;">
    ...
    <div style="position:relative; z-index:1;">
        <div class="flex items-start justify-between gap-4">
            <span class="metric-label" style="color: var(--dashboard-card-text-muted, rgba(255,255,255,0.88)); font-size:0.78rem; font-weight:700; text-transform:uppercase; letter-spacing:0.07em;">
                {{ $label }}
            </span>
            @if($icon)
                <div class="metric-icon" style="background: var(--dashboard-card-icon-bg, rgba(255,255,255,0.14)); color: var(--dashboard-card-icon-fg, #ffffff); border:1px solid var(--theme-border-soft, rgba(255,255,255,0.22)); border-radius:0.75rem; width:2.2rem; height:2.2rem; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                    <x-app-icon :name="$icon" />
                </div>
            @endif
        </div>
        <div class="metric-value" style="color: var(--dashboard-card-text, #ffffff); font-size:1.85rem; font-weight:900; line-height:1.2; margin-top:0.5rem; letter-spacing:-0.02em;">
            {{ $value }}
        </div>
        
        @if(isset($slot) && $slot->isNotEmpty())
            <div class="metric-note" style="color: var(--dashboard-card-text-muted, rgba(255,255,255,0.78)); font-size:0.75rem; font-weight:600; margin-top:0.3rem;">
                {{ $slot }}
            </div>
        @endif
    </div>
    ...
    @if($link)
        <div class="metric-link-row" style="border-top:1px solid var(--theme-border-soft, rgba(255,255,255,0.14)); margin-top:0.85rem; padding-top:0.7rem; position:relative; z-index:1;">
            <a href="{{ $link }}" class="metric-link group" style="color: var(--dashboard-card-text-muted, rgba(255,255,255,0.92)); font-size:0.8rem; font-weight:700; text-decoration:none; display:flex; align-items:center; gap:0.35rem;">
